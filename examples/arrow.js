...
goog.require('ol.style.CanvasShape');
...
var drawArrow = function(context, extent) {

  context.beginPath();
  context.moveTo(extent[0], extent[1]);
  context.lineTo(extent[2], 0);
  context.lineTo(extent[0], extent[3]);
  context.stroke();
};

var createArrowStyle = function createArrowStyle(cp) {
  return new ol.style.Style({
    geometry: new ol.geom.Point(cp.p),
    image: new ol.style.CanvasShape({
      renderFunction: drawArrow,
      extent: [-10, -5, 0, 5],
      rotateWithView: true,
...
var styleFunction = function(feature) {
  ...
  if (cp) {
    styles.push(createArrowStyle(cp));
  }
  return styles;
};
